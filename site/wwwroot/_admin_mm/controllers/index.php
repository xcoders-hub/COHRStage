	...
	case "get_home_list":
		...
		if( $result !== 0){
			foreach( $result as $row){
				...
				if (strpos($direct_link_to_file, '.jpg') !== false) {
					$myImage = '<img src="' . $direct_link_to_file . '"  style="max-width:100px; max-height:100px;margin:1em; float:left" >';
				} else {
				$myImage = '';
				}
				
				$all_links = $myImage . '<a href="'.$direct_link_to_file.'" target="_blank">'.$direct_link_to_file.'</a> ( CDN - Use This )<br/><br/>';
